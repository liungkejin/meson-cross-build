...
meson_build() {
  ...
  BUILD_OPTIONS="--buildtype=$BUILD_TYPE -Ddefault_library=$STATIC -Denable_tools=false -Denable_tests=false -Dlogging=false"
...
build_for_ohos() {
  ...
  meson_build ohos arm64-v8a "$cross_file" $@
}
